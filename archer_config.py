import os
# =====location of input/output files=====
# root of folder that contains the apps, automate_demultiplexing_logfiles and
# development_area scripts
# (2 levels up from this file)
document_root = "/usr/local/src/mokaguys/"
resources_root="/code/resources/"
logfile_folder = "logfiles"
script_logfile_folder = os.path.join(document_root,logfile_folder,"script_logfiles")
processed_runs_folder = os.path.join(document_root,logfile_folder,"processed_runs")
manifest_folder = os.path.join(document_root,logfile_folder,"manifest_files")

# DNA Nexus authentication token
nexus_api_key_file = "{document_root}/.dnanexus_auth_token".format(document_root=document_root)
with open(nexus_api_key_file, "r") as nexus_api:
	Nexus_API_Key = nexus_api.readline().rstrip()

# archerdx VM login
path_to_archerdx_pw = "{document_root}/.archerVM_pw".format(document_root=document_root)

download_location = os.path.join(document_root,"dx_downloads")
fastq_folder_path = "Data/Intensities/BaseCalls"
path_to_watch_folder = "/watched/aledjones\@nhs.net/FusionPlexPanSolidTumorv1_0" #folder made by RLH 20210622
source_command = " source %s" % os.path.join(resources_root,"dx-toolkit/environment")

success_statement = "all fastqs downloaded, transferred to server and deleted ok"

export_environment = "export DX_API_TOKEN=%s" % Nexus_API_Key

path_to_manifest_script = os.path.join(resources_root,"dxda_0.5.7/dxda/scripts","create_manifest.py")
path_to_filter_manifest_script = os.path.join(resources_root,"dxda_0.5.7/dxda/scripts","filter_manifest.py")
path_to_dx_download_client = os.path.join(resources_root,"dxda_0.5.7/","dx-download-agent-linux")